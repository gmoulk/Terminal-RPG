	...
	public:
		...
		void interact(){
			...
			while(i_want_to_buy){				
				...
				if(buyOption == 3){
					this->printSpells();
					cout << "What spell would you like to buy?(type from range 1 to " << this->spells.size() << " or 0 to cancel your action" << endl;
					int spellOption;
					cin >> spellOption;
					if(spellOption > 0){
						if(hs->buy(this->spells[spellOption - 1])){
							spells.erase(spells.begin() + spellOption - 1);
						}
					}
				}
				cout << "Would you like to buy something?(1 = yes / 0 = no)" << endl;
				cin >> i_want_to_buy;
				//cout << "What would you like to buy?(Choose between range " <<  << endl;
			}
		}
};
